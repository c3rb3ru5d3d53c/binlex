...
_ast = import_module("binlex_bindings.binlex.irs.ast")
...
class AstUnaryExpression(AstExpression):
    def __init__(self, op, value, ty):
        self._inner = _AstExpressionBinding.unary(op, _unwrap(value), _unwrap(ty))


class AstBinaryExpression(AstExpression):
    def __init__(self, op, lhs, rhs, ty):
        self._inner = _AstExpressionBinding.binary(op, _unwrap(lhs), _unwrap(rhs), _unwrap(ty))


class AstCastExpression(AstExpression):
    def __init__(self, op, value, ty):
        self._inner = _AstExpressionBinding.cast(op, _unwrap(value), _unwrap(ty))


class AstLoadExpression(AstExpression):
    def __init__(self, address_space, address, ty):
        self._inner = _AstExpressionBinding.load(_unwrap(address_space), _unwrap(address), _unwrap(ty))


class AstAddressOfExpression(AstExpression):
    def __init__(self, place, ty):
        self._inner = _AstExpressionBinding.address_of(_unwrap(place), _unwrap(ty))


class AstDereferenceExpression(AstExpression):
    def __init__(self, pointer, ty):
        self._inner = _AstExpressionBinding.dereference(_unwrap(pointer), _unwrap(ty))


class AstIndexExpression(AstExpression):
    def __init__(self, base, index, ty):
        self._inner = _AstExpressionBinding.index(_unwrap(base), _unwrap(index), _unwrap(ty))


class AstCompareExpression(AstExpression):
    def __init__(self, op, lhs, rhs, ty):
        self._inner = _AstExpressionBinding.compare(op, _unwrap(lhs), _unwrap(rhs), _unwrap(ty))


class AstSelectExpression(AstExpression):
    def __init__(self, condition, when_true, when_false, ty):
        self._inner = _AstExpressionBinding.select(
            _unwrap(condition),
            _unwrap(when_true),
            _unwrap(when_false),
            _unwrap(ty),
...
class AstDereferencePlace(AstPlace):
    def __init__(self, pointer, ty):
        self._inner = _AstPlaceBinding.dereference(_unwrap(pointer), _unwrap(ty))


class AstIndexPlace(AstPlace):
    def __init__(self, base, index, ty):
        self._inner = _AstPlaceBinding.index(_unwrap(base), _unwrap(index), _unwrap(ty))
