...
#[pyclass(name = "Client")]
pub struct Client {
    inner: binlex::clients::minio::Client,
}
...
impl Client {
    ...
    pub fn new(
        ...
        let inner =
            binlex::clients::minio::Client::new(endpoint, access_key, secret_key, secure)
...
pub fn minio_init(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("binlex_bindings.binlex.clients.minio", m)?;
    m.setattr("__name__", "binlex_bindings.binlex.clients.minio")?;
    Ok(())
}
