...
#[pyclass(name = "Client")]
pub struct Client {
    inner: binlex::clients::server::Client,
}
...
impl Client {
    ...
    pub fn new(
        ...
        let inner = binlex::clients::server::Client::new(inner_config, url, verify, compression)
...
fn map_client_error(error: binlex::clients::server::Error) -> PyErr {
    match error {
        binlex::clients::server::Error::InvalidConfiguration(message) => {
...
#[pymodule]
#[pyo3(name = "server")]
pub fn server_init(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Client>()?;
    py.import("sys")?
        .getattr("modules")?
        .set_item("binlex_bindings.binlex.clients.server", m)?;
    m.setattr("__name__", "binlex_bindings.binlex.clients.server")?;
    Ok(())
}
