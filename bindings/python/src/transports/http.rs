...
#[pyclass(name = "Client")]
pub struct Client {
    inner: binlex::transports::http::Client,
}
...
impl Client {
    ...
    pub fn new(
        ...
        let inner = binlex::transports::http::Client::new(inner_config, url, verify, compression)
...
fn map_client_error(error: binlex::transports::http::Error) -> PyErr {
    match error {
        binlex::transports::http::Error::InvalidConfiguration(message) => {
...
#[pymodule]
#[pyo3(name = "http")]
pub fn http_init(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    m.add_class::<Client>()?;
    py.import("sys")?
        .getattr("modules")?
        .set_item("binlex_bindings.binlex.transports.http", m)?;
    m.setattr("__name__", "binlex_bindings.binlex.transports.http")?;
    Ok(())
}
