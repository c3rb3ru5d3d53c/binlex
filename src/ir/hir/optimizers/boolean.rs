...
fn simplify_boolean_compare(
    ...
    match (op, boolean_value) {
        ...
        _ => HirExpression::Compare {
            op,
            lhs: Box::new(value),
            rhs: Box::new(HirExpression::Value(crate::ir::hir::HirValue::Boolean(
                boolean_value,
            ))),
            ty,
        },
...
fn expression_is_boolean(expression: &HirExpression) -> bool {
    match expression {
        HirExpression::Value(crate::ir::hir::HirValue::Boolean(_)) => true,
        HirExpression::Value(crate::ir::hir::HirValue::Named { ty, .. }) => {
...
fn boolean_constant_value(expression: &HirExpression) -> Option<bool> {
    match expression {
        HirExpression::Value(crate::ir::hir::HirValue::Boolean(value)) => Some(*value),
        HirExpression::Value(crate::ir::hir::HirValue::Integer { value, bits: 1 }) => {
            Some(*value != 0)
        }
        _ => None,
    }
