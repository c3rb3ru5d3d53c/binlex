...
fn eval_expression_wide(
    ...
    match expression {
        ...
        LirExpression::Read(location) => match location.as_ref() {
            ...
            LirLocation::Temporary { id, bits } => mask_to_bits_wide(
        ...
        LirExpression::Unary { op, arg, bits } => {
            ...
            match op {
                ...
                LirOperationUnary::CountLeadingZeros => BigUint::from(count_leading_zeros_wide(
                    mask_to_bits_wide(value, *bits),
                    *bits,
                ) as u64),
                LirOperationUnary::CountTrailingZeros => BigUint::from(count_trailing_zeros_wide(
                    mask_to_bits_wide(value, *bits),
                    *bits,
                ) as u64),
                LirOperationUnary::PopCount => BigUint::from(
                    value
                        .to_bytes_le()
                        .iter()
        ...
        LirExpression::Binary {
            ...
            let value = match op {
                LirOperationBinary::Add => left + right,
                ...
                LirOperationBinary::Mul => left * right,
                LirOperationBinary::And => left & right,
                LirOperationBinary::Or => left | right,
                LirOperationBinary::Xor => left ^ right,
                LirOperationBinary::Shl => left << shift_amount(&right),
                LirOperationBinary::LShr => left >> shift_amount(&right),
                LirOperationBinary::AShr => arithmetic_shift_right_wide(left, right, *bits),
                LirOperationBinary::MinUnsigned => left.min(right),
                LirOperationBinary::MinSigned => match compare_signed_wide(&left, &right, *bits) {
                    std::cmp::Ordering::Greater => right,
                    _ => left,
                },
                LirOperationBinary::MaxUnsigned => left.max(right),
                LirOperationBinary::MaxSigned => match compare_signed_wide(&left, &right, *bits) {
        ...
        LirExpression::Cast { op, arg, bits } => {
            ...
            match op {
                ...
                | LirOperationCast::Truncate => mask_to_bits_wide(value, *bits),
        ...
        LirExpression::Compare {
            ...
            let right = eval_expression_wide(right, registers, flags, memory, temporaries);
            let value = match op {
                LirOperationCompare::Eq => left == right,
                LirOperationCompare::Ne => left != right,
                LirOperationCompare::Ult => left < right,
                LirOperationCompare::Ule => left <= right,
                LirOperationCompare::Ugt => left > right,
                LirOperationCompare::Uge => left >= right,
                LirOperationCompare::Slt => {
                    compare_signed_wide(&left, &right, compare_bits) == std::cmp::Ordering::Less
                }
                LirOperationCompare::Sle => {
                    compare_signed_wide(&left, &right, compare_bits) != std::cmp::Ordering::Greater
                }
                LirOperationCompare::Sgt => {
                    compare_signed_wide(&left, &right, compare_bits) == std::cmp::Ordering::Greater
                }
                LirOperationCompare::Sge => {
                    compare_signed_wide(&left, &right, compare_bits) != std::cmp::Ordering::Less
                }
        ...
        LirExpression::Extract { arg, lsb, bits } => mask_to_bits_wide(
...
fn return_expression_encodes_target(expression: &LirExpression) -> bool {
    matches!(
        expression,
        LirExpression::Load { .. } | LirExpression::Read(_) | LirExpression::Const { .. }
    )
}
...
fn eval_expression(
    ...
    match expression {
        ...
        LirExpression::Read(location) => match location.as_ref() {
            ...
            LirLocation::Flag { name, .. } => *flags.get(name).expect("known flag read") & 1,
            LirLocation::Temporary { id, bits } => {
        ...
        LirExpression::Unary { op, arg, bits } => {
            ...
            match op {
                LirOperationUnary::Not => mask_to_bits(!value, *bits),
                LirOperationUnary::Neg => mask_to_bits((0u128).wrapping_sub(value), *bits),
                LirOperationUnary::ByteSwap => mask_to_bits(byte_swap(value, *bits), *bits),
                LirOperationUnary::CountLeadingZeros => mask_to_bits(
                    count_leading_zeros(mask_to_bits(value, *bits), *bits) as u128,
                    *bits,
                ),
                LirOperationUnary::CountTrailingZeros => mask_to_bits(
        ...
        LirExpression::Binary {
            ...
            let value = match op {
                LirOperationBinary::Add => left.wrapping_add(right),
                LirOperationBinary::Sub => left.wrapping_sub(right),
                LirOperationBinary::Mul => left.wrapping_mul(right),
                ...
                LirOperationBinary::FAdd => u128::from(
                    (f64::from_bits(left as u64) + f64::from_bits(right as u64)).to_bits(),
                ),
                LirOperationBinary::FSub => u128::from(
                    (f64::from_bits(left as u64) - f64::from_bits(right as u64)).to_bits(),
                ),
                LirOperationBinary::FMul => u128::from(
                    (f64::from_bits(left as u64) * f64::from_bits(right as u64)).to_bits(),
                ),
                LirOperationBinary::FDiv => u128::from(
                    (f64::from_bits(left as u64) / f64::from_bits(right as u64)).to_bits(),
                ),
                LirOperationBinary::And => left & right,
                LirOperationBinary::Or => left | right,
                LirOperationBinary::Xor => left ^ right,
                LirOperationBinary::Shl => left.wrapping_shl((right & u32::MAX as u128) as u32),
                LirOperationBinary::LShr => left.wrapping_shr((right & u32::MAX as u128) as u32),
                ...
                LirOperationBinary::AShr => arithmetic_shift_right(left, right, *bits),
                LirOperationBinary::MinUnsigned => left.min(right),
                LirOperationBinary::MinSigned => match compare_signed(left, right, *bits) {
                    std::cmp::Ordering::Greater => right,
                    _ => left,
                },
                LirOperationBinary::MaxUnsigned => left.max(right),
                LirOperationBinary::MaxSigned => match compare_signed(left, right, *bits) {
                    std::cmp::Ordering::Less => right,
                    _ => left,
                },
        ...
        LirExpression::Cast { op, arg, bits } => {
            ...
            match op {
                LirOperationCast::ZeroExtend
                | LirOperationCast::Bitcast
                | LirOperationCast::Truncate => mask_to_bits(value, *bits),
                LirOperationCast::SignExtend => {
        ...
        LirExpression::Compare {
            ...
            match op {
                LirOperationCompare::Eq => (left == right) as u128,
                LirOperationCompare::Ne => (left != right) as u128,
                LirOperationCompare::Ult => (left < right) as u128,
                LirOperationCompare::Ule => (left <= right) as u128,
                LirOperationCompare::Ugt => (left > right) as u128,
                LirOperationCompare::Uge => (left >= right) as u128,
                LirOperationCompare::Slt => {
        ...
        LirExpression::Extract { arg, lsb, bits } => mask_to_bits(
...
fn arg_bits(expression: &LirExpression) -> u16 {
    match expression {
        ...
        LirExpression::Read(location) => location.bits(),
    }
}
            ...
            match dst {
                ...
                LirLocation::Flag { name, .. } => {
                    if !flags.contains(name)
                        && !matches!(
                            effect.expression(),
                            Some(LirExpression::Undefined { .. } | LirExpression::Poison { .. })
                        )
                    {
...
fn is_directly_undefined(expression: &LirExpression) -> bool {
    matches!(
        expression,
        LirExpression::Undefined { .. } | LirExpression::Poison { .. }
    )
}
