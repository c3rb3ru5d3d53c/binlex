...
fn interpret_arm64_semantics(
    ...
    let post = Arm64CpuState {
        ...
        pc: match semantics.terminator {
            ...
            LirTerminator::Jump { ref target } => relocate_code_target(eval_expression(
                target,
                &pre.registers,
                &temporaries,
                &pre_memory,
            ) as u64),
            LirTerminator::Call { ref target, .. } => relocate_code_target(eval_expression(
                target,
                &pre.registers,
                &temporaries,
                &pre_memory,
            ) as u64),
            LirTerminator::Return { ref expression } => expression
                .as_ref()
                .map(|expression| {
...
fn relocate_code_like_result(
    ...
    if matches!(expression, LirExpression::Const { .. })
...
fn eval_expression(
    ...
    match expression {
        ...
        LirExpression::Read(location) => match location.as_ref() {
            LirLocation::Register { name, bits } | LirLocation::Flag { name, bits } => {
        ...
        LirExpression::Binary {
            ...
            let value = match op {
                ...
                LirOperationBinary::FAdd => float_add_with_width(left, right, *bits),
                LirOperationBinary::FSub => float_sub_with_width(left, right, *bits),
                LirOperationBinary::FMul => float_mul_with_width(left, right, *bits),
                LirOperationBinary::FDiv => float_div_with_width(left, right, *bits),
                LirOperationBinary::UMulHigh => unsigned_mul_high_with_width(left, right, *bits),
                LirOperationBinary::SMulHigh => signed_mul_high_with_width(left, right, *bits),
                ...
                LirOperationBinary::Shl => left.wrapping_shl(right as u32),
                LirOperationBinary::LShr => left.wrapping_shr(right as u32),
                LirOperationBinary::AShr => {
                    let signed = sign_extend(left, *bits);
                    ((signed as i128) >> (right as u32)) as u128
                }
                LirOperationBinary::RotateRight => rotate_right_with_width(left, right, *bits),
        ...
        LirExpression::Unary { op, arg, bits } => {
            ...
            let value = match op {
                LirOperationUnary::Not => !arg,
                LirOperationUnary::Abs => float_abs_with_width(arg, *bits),
                LirOperationUnary::Neg => float_neg_with_width(arg, *bits),
                LirOperationUnary::BitReverse => bit_reverse_with_width(arg, *bits),
                LirOperationUnary::CountLeadingZeros => count_leading_zeros_with_width(arg, *bits),
                LirOperationUnary::PopCount => (mask_to_bits(arg, *bits)).count_ones() as u128,
                LirOperationUnary::ByteSwap => byte_swap_with_width(arg, *bits),
        ...
        LirExpression::Cast { op, arg, bits } => {
            ...
            let value = match op {
                LirOperationCast::ZeroExtend => arg_value,
                LirOperationCast::SignExtend => sign_extend(arg_value, arg_bits),
                LirOperationCast::FloatToInt => float_to_int_bits(arg_value, *bits),
                LirOperationCast::FloatToUInt => float_to_uint_bits(arg_value, *bits),
                LirOperationCast::IntToFloat => int_to_float_bits(arg_value, arg_bits, *bits),
                LirOperationCast::UIntToFloat => uint_to_float_bits(arg_value, arg_bits, *bits),
        ...
        match effect {
            ...
            LirEffect::Intrinsic { outputs, .. } => {
                for output in outputs {
                    if let LirLocation::Register { name, .. } | LirLocation::Flag { name, .. } =
                        output
                    {
                        if !registers.contains(name) {
                            registers.push(name.clone());
            ...
            LirEffect::AtomicCmpXchg { observed, .. } => {
                if let LirLocation::Register { name, .. } | LirLocation::Flag { name, .. } =
                    observed
                {
                    if !registers.contains(name) {
...
fn expression_bits(expression: &LirExpression) -> u16 {
    match expression {
        ...
        | LirExpression::Concat { bits, .. } => *bits,
