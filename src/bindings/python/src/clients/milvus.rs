...
#[pyclass(name = "Client")]
pub struct Client {
    inner: binlex::clients::milvus::Client,
}

fn field_type_from_python(
    field: &Bound<'_, PyDict>,
) -> PyResult<binlex::clients::milvus::FieldType> {
    let kind: String = field
        .get_item("kind")?
        .ok_or_else(|| pyo3::exceptions::PyValueError::new_err("field is missing kind"))?
        .extract()?;
    match kind.as_str() {
        "varchar" => Ok(binlex::clients::milvus::FieldType::VarChar),
        "int64" => Ok(binlex::clients::milvus::FieldType::Int64),
        "bool" => Ok(binlex::clients::milvus::FieldType::Bool),
        "float_vector" => {
            let dimensions: usize = field
                ...
                .extract()?;
            Ok(binlex::clients::milvus::FieldType::FloatVector { dimensions })
        }
...
fn field_schema_from_python(
    field: &Bound<'_, PyDict>,
) -> PyResult<binlex::clients::milvus::FieldSchema> {
    ...
    let primary_key = field
        ...
        .transpose()?
        .unwrap_or(false);
    Ok(binlex::clients::milvus::FieldSchema {
        name,
...
impl Client {
    ...
    pub fn new(uri: String, token: Option<String>) -> PyResult<Self> {
        let inner = binlex::clients::milvus::Client::new(uri, token)
...
pub fn milvus_init(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    py.import("sys")?
        .getattr("modules")?
        .set_item("binlex_bindings.binlex.clients.milvus", m)?;
    m.setattr("__name__", "binlex_bindings.binlex.clients.milvus")?;
    Ok(())
}
