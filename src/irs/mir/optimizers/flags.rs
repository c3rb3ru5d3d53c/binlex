...
fn terminator_uses(terminator: &crate::irs::mir::MirTerminator) -> Vec<String> {
    match terminator {
        crate::irs::mir::MirTerminator::Jump { arguments, .. }
        | crate::irs::mir::MirTerminator::Return { values: arguments } => {
            arguments.iter().filter_map(named_value).collect()
        }
