...
fn expression_bits(expression: &LirExpression) -> u16 {
    match expression {
        ...
        LirExpression::Read(location) => match location.as_ref() {
            crate::irs::lir::LirLocation::Register { bits, .. }
            | crate::irs::lir::LirLocation::Flag { bits, .. }
            | crate::irs::lir::LirLocation::ProgramCounter { bits }
            | crate::irs::lir::LirLocation::Temporary { bits, .. }
            | crate::irs::lir::LirLocation::Memory { bits, .. }
            | crate::irs::lir::LirLocation::IndexedMemory { bits, .. }
            | crate::irs::lir::LirLocation::StackMemory { bits, .. } => *bits,
        },
    }
}
...
fn coerce_expression_width(expression: LirExpression, bits: u16) -> LirExpression {
    ...
    if current_bits < bits {
        ...
        LirExpression::Cast {
            op: crate::irs::lir::LirOperationCast::Truncate,
            arg: Box::new(expression),
            bits,
        }
    }
...
fn normalize_shift_binary(
    ...
    match op {
        ...
        | crate::irs::lir::LirOperationBinary::LShr
        | crate::irs::lir::LirOperationBinary::AShr => (left, coerce_expression_width(right, bits)),
        _ => (left, right),
    }
}
...
fn prepare_terminator(
    ...
    match terminator {
        ...
        LirTerminator::Return { expression } => {
            let post_effect_snapshots = HashMap::new();
            let post_effect_load_snapshots = HashMap::new();
            LirTerminator::Return {
                expression: expression.as_ref().map(|expression| {
                    prepare_expression(
                        expression,
                        &post_effect_snapshots,
                        &post_effect_load_snapshots,
                    )
                }),
            }
        }
        ...
        let lir = LirInstruction {
            ...
            effects: vec![LirEffect::Set {
                ...
                expression: LirExpression::Compare {
                    op: crate::irs::lir::LirOperationCompare::Uge,
