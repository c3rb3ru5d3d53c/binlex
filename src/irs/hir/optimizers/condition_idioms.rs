use crate::irs::hir::optimizers::algebraic::simplify_algebraic_expression;
...
fn is_zero_expression(expression: &HirExpression) -> bool {
    matches!(
        expression,
        HirExpression::Value(crate::irs::hir::HirValue::Integer { value: 0, .. })
    ) || matches!(
        expression,
        HirExpression::Value(crate::irs::hir::HirValue::Boolean(false))
    )
}
