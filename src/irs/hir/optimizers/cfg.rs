use crate::irs::hir::optimizers::boolean::{optimize_boolean, simplify_logical_not};
use crate::irs::hir::{
...
fn collect_assigned_names_in_statement(statement: &HirStatement, names: &mut BTreeSet<String>) {
    match statement {
        HirStatement::Assign {
            target: crate::irs::hir::HirPlace::Named { name, .. },
            ..
        } => {
...
fn expression_references_names(expression: &HirExpression, names: &BTreeSet<String>) -> bool {
    match expression {
        HirExpression::Value(crate::irs::hir::HirValue::Named { name, .. }) => names.contains(name),
...
fn place_references_names(place: &crate::irs::hir::HirPlace, names: &BTreeSet<String>) -> bool {
    match place {
        crate::irs::hir::HirPlace::Named { name, .. } => names.contains(name),
        crate::irs::hir::HirPlace::Dereference { pointer, .. }
        | crate::irs::hir::HirPlace::Memory {
            address: pointer, ..
        } => expression_references_names(pointer, names),
        crate::irs::hir::HirPlace::Index { base, index, .. } => {
