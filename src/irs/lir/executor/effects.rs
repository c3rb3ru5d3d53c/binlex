...
use crate::irs::lir::{LirEffect, LirExpression, LirLocation};
...
impl LirExecutor {
    ...
    ) -> Result<crate::irs::lir::executor::expressions::EvaluatedValue, LirExecutorError> {
        if value.deps.is_empty() && value.value.get_size() <= 64 {
            if let Some(concrete) = state
                .backend()
                .eval_bv_u64(state.solver_constraints(), &value.value)?
            {
                return Ok(crate::irs::lir::executor::expressions::EvaluatedValue {
                    value: state
                        .backend()
        ...
        match effect {
            ...
            LirEffect::Pop { stack, dst } => {
                ...
                self.write_location(
                    ...
                    instruction,
                    dst,
                    crate::irs::lir::executor::expressions::EvaluatedValue {
                        value: cell.value,
    ...
    pub(crate) fn read_location(
        ...
        match location {
            LirLocation::Register { name, bits } => {
                ...
                Ok(crate::irs::lir::executor::expressions::EvaluatedValue {
                    value: cell.value,
                    deps: cell.def_id.into_iter().collect(),
                })
            }
            ...
            LirLocation::Memory { addr, bits, .. } => {
                ...
                Ok(crate::irs::lir::executor::expressions::EvaluatedValue { value, deps })
            }
