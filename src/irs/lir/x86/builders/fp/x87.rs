use super::*;
use crate::irs::lir::LirAddressSpace;
...
fn x87_compare_effects(rhs: LirExpression, pop_count: u8) -> Vec<LirEffect> {
    let lhs = x87_stack_expr(0);
    let unordered = common::compare(LirOperationCompare::Unordered, lhs.clone(), rhs.clone());
    let equal = common::compare(LirOperationCompare::Oeq, lhs.clone(), rhs.clone());
    let less = common::compare(LirOperationCompare::Olt, lhs, rhs);
    let mut effects = vec![
        LirEffect::Set {
            dst: x87_flag("c0"),
...
fn x87_compare_to_cpu_flags(rhs: LirExpression, pop_count: u8) -> Vec<LirEffect> {
    let lhs = x87_stack_expr(0);
    let unordered = common::compare(LirOperationCompare::Unordered, lhs.clone(), rhs.clone());
    let equal = common::compare(LirOperationCompare::Oeq, lhs.clone(), rhs.clone());
    let less = common::compare(LirOperationCompare::Olt, lhs, rhs);
...
fn x87_store_effect(
    ...
    match operand.kind {
        X86OperandKind::Register => Some(LirEffect::Set {
...
fn x87_exchange(operands: &[X86OperandView]) -> Option<Vec<LirEffect>> {
...
fn x87_push_effects(value: LirExpression) -> Vec<LirEffect> {
...
fn x87_push_with_replacement(pushed: LirExpression, replaced_st0: LirExpression) -> Vec<LirEffect> {
    let mut effects = Vec::new();
...
fn x87_pop_effects(count: u8) -> Vec<LirEffect> {
    if count == 0 {
        return Vec::new();
    }
    let mut effects = Vec::new();
    for index in 0..8 {
        ...
        let expression = if src < 8 {
            ...
            LirExpression::Undefined { bits: X87_BITS }
        };
        effects.push(LirEffect::Set {
...
fn x87_pop_with_replacement(target_index: u8, replacement: LirExpression) -> Vec<LirEffect> {
    let mut effects = Vec::new();
    for index in 0..8u8 {
...
fn x87_rotate_top(increment: bool) -> Vec<LirEffect> {
    let mut effects = Vec::new();
    for index in 0..8u8 {
...
fn x87_float_operand(machine: Architecture, operand: &X86OperandView) -> Option<LirExpression> {
    if let Some(index) = x87_stack_index(operand) {
...
fn x87_int_operand(machine: Architecture, operand: &X86OperandView) -> Option<LirExpression> {
    let raw = operand_expr(machine, operand)?;
    let bits = x87_operand_bits(operand)?;
    Some(match bits {
...
fn x87_bcd_operand(machine: Architecture, operand: &X86OperandView) -> Option<LirExpression> {
    let raw = operand_expr(machine, operand)?;
    let bits = x87_operand_bits(operand)?;
    if bits != 80 {
        return None;
...
fn x87_store_bits(value: LirExpression, bits: u16) -> LirExpression {
    match bits {
        32 => x87_expr_intrinsic("store_f32", vec![value]),
        64 => x87_expr_intrinsic("store_f64", vec![value]),
        80 => value,
        _ => LirExpression::Undefined { bits },
    }
}

fn x87_bcd_store(value: LirExpression) -> LirExpression {
    x87_expr_intrinsic("store_bcd", vec![value])
}

fn x87_memory_addr(machine: Architecture, operand: &X86OperandView) -> Option<LirExpression> {
    let location = operand_location(machine, operand)?;
    match location {
        LirLocation::Memory { addr, .. } => Some(*addr),
        _ => None,
    }
}
...
fn x87_stack_location(index: u8) -> LirLocation {
    common::reg(format!("x87_st{index}"), X87_BITS)
}

fn x87_stack_expr(index: u8) -> LirExpression {
    LirExpression::Read(Box::new(x87_stack_location(index)))
}

fn x87_top_location() -> LirLocation {
    common::reg("x87_top", 3)
}

fn x87_top_expr() -> LirExpression {
    LirExpression::Read(Box::new(x87_top_location()))
}

fn x87_flag(name: &str) -> LirLocation {
    common::flag(&format!("x87_{name}"))
}

fn x87_const(name: &str) -> LirExpression {
...
fn x87_expr_intrinsic(name: &str, args: Vec<LirExpression>) -> LirExpression {
    LirExpression::Intrinsic {
...
fn x87_status_word() -> LirExpression {
    let top_bits = LirExpression::Cast {
        op: LirOperationCast::ZeroExtend,
        arg: Box::new(x87_top_expr()),
        bits: 16,
    };
    let top_shifted = LirExpression::Binary {
    ...
    for (name, bit) in [("c0", 8), ("c1", 9), ("c2", 10), ("c3", 14)] {
        let shifted = LirExpression::Binary {
            ...
            left: Box::new(LirExpression::Cast {
                op: LirOperationCast::ZeroExtend,
                arg: Box::new(LirExpression::Read(Box::new(x87_flag(name)))),
                bits: 16,
            }),
...
fn x87_init_effects() -> Vec<LirEffect> {
    let mut effects = Vec::new();
    for index in 0..8u8 {
        effects.push(LirEffect::Set {
            dst: x87_stack_location(index),
...
fn x87_conditional_move(mnemonic: &str, operands: &[X86OperandView]) -> Option<Vec<LirEffect>> {
...
fn x87_intrinsic_with_flag_outputs(name: &str, args: Vec<LirExpression>) -> Vec<LirEffect> {
...
fn x87_flag_only_intrinsic(name: &str, args: Vec<LirExpression>) -> Vec<LirEffect> {
    vec![LirEffect::Intrinsic {
        name: format!("x86.x87.{name}"),
        args,
        outputs: vec![
