    ...
    while (true){
        ...
        while(true) {
            ...
            if (result != true){
                ...
                fprintf(stderr, "*** Decompile error rejected block: 0x%x\n", myself.pc);
                ClearTrait(&b_trait);
                ClearTrait(&i_trait);
                ClearTrait(&f_trait);
                break;
            ...
            if (suspicious_instructions > 2){
                fprintf(stderr, "*** Suspicious instructions rejected block: 0x%x\n", insn->address);
                ClearTrait(&b_trait);
                ClearTrait(&i_trait);
                ClearTrait(&f_trait);
                break;
...
void Decompiler::LinearDisassemble(void* data, size_t data_size, size_t offset, uint index) {
    ...
    PRINT_DEBUG("LinearDisassemble: offset = 0x%x data_size = %" PRId64 " bytes\n", offset, data_size);
