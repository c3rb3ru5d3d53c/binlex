...
impl LanceDB {
    ...
    pub fn upsert(
        ...
        let table_name = table_name(collection, architecture);
    ...
    pub fn upsert_rows(
        ...
        let table = self
            ...
            .block_on(connection.open_table(&table_name).execute())
            .map_err(|error| Error::Lance(error.to_string()))?;
        for chunk in rows.chunks(UPSERT_ROWS_CHUNK_SIZE) {
            let delete_predicate = delete_predicate_for_rows(chunk);
...
fn table_name(collection: Collection, architecture: &str) -> String {
    format!("{}__{}", collection.as_str(), architecture)
}
