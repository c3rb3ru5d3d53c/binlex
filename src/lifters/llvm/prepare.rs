...
fn prepare_effect(effect: &SemanticEffect) -> SemanticEffect {
    match effect {
        ...
        SemanticEffect::MemorySet {
            ...
            space: space.clone(),
            addr: prepare_expression(addr),
            value: prepare_expression(&coerce_expression_width(value.clone(), *element_bits)),
            count: prepare_expression(count),
            element_bits: *element_bits,
            decrement: prepare_expression(decrement),
        },
        SemanticEffect::MemoryCopy {
            ...
            src_addr: prepare_expression(src_addr),
            dst_space: dst_space.clone(),
            dst_addr: prepare_expression(dst_addr),
            count: prepare_expression(count),
            element_bits: *element_bits,
            decrement: prepare_expression(decrement),
        },
        SemanticEffect::AtomicCmpXchg {
            ...
            space: space.clone(),
            addr: prepare_expression(addr),
            expected: prepare_expression(&coerce_expression_width(expected.clone(), *bits)),
            desired: prepare_expression(&coerce_expression_width(desired.clone(), *bits)),
            bits: *bits,
