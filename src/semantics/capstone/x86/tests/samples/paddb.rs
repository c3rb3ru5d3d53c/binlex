...
fn status_samples() -> Vec<X86RuntimeSample> {
    ...
    {
        let cases = [
            ("paddb xmm0, xmm1", vec![0x66, 0x0f, 0xfc, 0xc1]),
            ("vpaddb xmm0, xmm2, xmm1", vec![0xc5, 0xe9, 0xfc, 0xc1]),
        ];
...
fn conformance_samples() -> Vec<X86RuntimeSample> {
    let mut samples = Vec::new();
    {
        ...
        samples.push(X86RuntimeSample {
            mnemonic: "paddb",
            instruction: "paddb xmm0, xmm1",
            architecture: Architecture::AMD64,
            bytes: (&[0x66, 0x0f, 0xfc, 0xc1]).to_vec(),
...
#[test]
fn paddb_semantics_regressions_stay_complete() {
    let samples = status_samples();
    assert_runtime_sample_statuses(&samples);
}

#[test]
fn paddb_semantics_match_unicorn_transitions() {
