...
class CILDecompiler {
    ...
    public:
        ...
        bool Decompile(void *data, int data_size, int index){
            const unsigned char *pc = (const unsigned char *)data;
            char *bytes = NULL;
            // BUG: When memory is allocated for the trait we are not taking into account
            //      the new line character we add using traits_nl function at the end of a
            //      block or a function.
            //      Assuming that the minimum size of a CIL instruction is 1. I choose the
            //      worst scenario where each opcode is the end of the block.
            //      I need to allocate an extra byte per byte
            char *traits = (char *)malloc(data_size * 2 + data_size + data_size + 1);
            ...
            if (type == CIL_DECOMPILER_TYPE_FUNCS){
                sections[index].function_traits = (char *)calloc(1, strlen(traits)+1);
        ...
        ~CILDecompiler(){
            for (int i = 0; i < CIL_DECOMPILER_MAX_SECTIONS; i++){
                if (type == CIL_DECOMPILER_TYPE_FUNCS && sections[i].function_traits != NULL){
                    free(sections[i].function_traits);
                }
                if (type == CIL_DECOMPILER_TYPE_BLCKS && sections[i].block_traits != NULL){
